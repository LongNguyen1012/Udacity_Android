...
public class EditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCostEditText = (EditText) findViewById(R.id.edit_inventory_cost);
    ...
    private void setupSpinner() {
        ...
        mConditionSpinner.setAdapter(genderSpinnerAdapter);
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    if (selection.equals(getString(R.string.condition_new))) {
                        mCondition = InventoryContract.InventoryEntry.CONDITION_NEW; // Male
                    } else if (selection.equals(getString(R.string.condition_used))) {
                        mCondition = InventoryContract.InventoryEntry.CONDITION_USED; // Female
                    } else {
    ...
    private void insertInventory() {
        ...
        values.put(InventoryContract.InventoryEntry.COLUMN_INVENTORY_COST, cost);
        values.put(InventoryContract.InventoryEntry.COLUMN_INVENTORY_PRICE, price);
        values.put(InventoryContract.InventoryEntry.COLUMN_INVENTORY_CONDITION, mCondition);
        values.put(InventoryContract.InventoryEntry.COLUMN_INVENTORY_QUANTITY, quantity);

        long newRowId = db.insert(InventoryContract.InventoryEntry.TABLE_NAME, null, values);

        if (newRowId == -1) {
            Toast.makeText(this, "Error inserting inventory", Toast.LENGTH_SHORT).show();
        } else {
