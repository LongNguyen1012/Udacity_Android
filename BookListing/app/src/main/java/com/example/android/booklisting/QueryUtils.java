    ...
    public static ArrayList<Book> extractFeatureFromjson(String bookJSON) {
        ...
        try {
            ...
            for (int i = 0; i < itemsArray.length(); i++) {
                JSONObject currentBooks = itemsArray.getJSONObject(i);
                ...
                //Convert author JSON array to array list
                if (authorArray != null) {
                    StringBuilder authorString = new StringBuilder();
                    for (int j = 0; j < authorArray.length(); j++) {
                        authorString.append(authorArray.getString(j));
                        authorString.append(", ");
                    }
                    String b = authorString.toString();
                    authorStringFinal = b.substring(0,b.length()-2);
                } else {
                    authorStringFinal = "Not found";
                }

                // Create a new {@link Book} object with the rating, title, author, published date,
                // and url from the JSON response.
                Book book = new Book(rating, title, authorStringFinal, date, url);
                books.add(book);
            }
